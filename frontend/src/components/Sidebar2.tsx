...
export default function Forms({ onClose, updateAnimals }: FormsProps) {  
            ...
            <li className="flex">
              {line.map((tag, index) => (
                <>
                  ...
                  <Input type="checkbox" value={tag.isMarked} onChange={event => handleTagMark(event, indexLine * cntPerLine + index)} className="h-5 hover:cursor-pointer w-1/6"/>
                </>
              ))}
            </li>
          ))}
  ...
  function getAllTags() {
    tagApi.getAll().then((res) => setTags(res.map(tag => {
      tag.isMarked = false;
      return tag;
    })));
  }
        ...
        <button type="button" onClick={onClose}>
          ...
          />
            ...
            <div className="overflow-scroll">
                <Label>Cidade</Label>
                <select name="cityIds" className="border rounded-2xl p-2 w-full flex flex-col grid-rows-2 gap-5 bg-white text-sm">
                    <option></option>
                    {cities.map((city) => (
            ...
            <div>
                ...
                <select name="ownerKind" className="border rounded-2xl p-2 w-full flex flex-col grid-rows-2 gap-5 bg-white text-sm">
                    <option></option>
                    <option key="user" value="user">Pessoa Física</option>
                    <option key="ong" value="ong">ONG</option>
                </select>
            </div>
            <div>
                <Label>Espécie</Label>
                <select name="speciesIds" className="border rounded-2xl p-2 w-full flex flex-col grid-rows-2 gap-5 bg-white text-sm">
                  <option></option>
                  {species.map((specie) => (
            ...
            <div>
                <Label>Sexo</Label>
                <select name="genders" className="border rounded-2xl p-2 w-full flex flex-col grid-rows-2 gap-5 bg-white text-sm">
                    <option></option>
            ...
            <div className="flex flex-row p-4 justify-start">
                <div className="basis-1/2 flex flex-row">
                    ...
                    <Input name="isVerm" type="checkbox" className="h-5 hover:cursor-pointer"/>
                </div>
                <div className="basis-1/2 flex flex-row">
                    <Label>Castrado</Label>
                    <Input name="isCast" type="checkbox" className="h-5 hover:cursor-pointer"/>
                </div>
            </div>
